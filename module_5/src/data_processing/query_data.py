"""
Module to hold and run query data.
"""
...
# Above due to set-up of Query object
...
class Query():
    ...
    def run_query(self):
        ...
        with connection.cursor() as c:
            self.find_ct_applicants_f26(c)
            self.find_pct_intl(c)
            self.find_averages(c)
            self.find_avg_us_gpa_f26(c)
            self.find_pct_accepted_f25(c)
            self.find_avg_accept_gpa_f26(c)
            self.find_ct_jhu_ms_cs(c)
            self.find_ct_select_phd_cs(c)
            self.find_ct_select_phd_cs_llm(c)
            self.find_prog_most_accept(c)
            self.find_prog_most_reject(c)
            self.find_uni_most_accept(c)
            self.find_uni_most_reject(c)
    ...
    def find_ct_applicants_f26(self, c):
        """
        Find the count of entries that have applied for Fall 2026.
        """
    ...
    def find_pct_intl(self, c):
        """
        Find the percentage of entries from international students.
        """
    ...
    def find_averages(self, c):
        """
        Find the average GPA, GRE, GRE V, and GRE AW of applicants.
        """
        ...
        self.avg_gpa, self.avg_gre, self.avg_gre_v, \
            self.avg_gre_aw = f"{row[0]:.2f}", \
                f"{row[1]:.2f}", f"{row[2]:.2f}", f"{row[3]:.2f}"
        print(f"Avg GPA: {self.avg_gpa}, Avg GRE: {self.avg_gre}, "
                f"Avg GRE V: {self.avg_gre_v}, Avg GRE AW: {self.avg_gre_aw}")

    def find_avg_us_gpa_f26(self, c):
        """
        Find the average GPA of American students in Fall 2026.
        """
    ...
    def find_pct_accepted_f25(self, c):
        """
        Find the percent of entries in Fall 2025 that are Acceptances.
        """
    ...
    def find_avg_accept_gpa_f26(self, c):
        """
        Find the average GPA of accepted students in Fall 2026.
        """
        c.execute("""SELECT COALESCE(avg(gpa), 0)
                FROM results
    ...
    def find_ct_jhu_ms_cs(self, c):
        """
        Find the count of applicants to JHU for a Masters in Computer Science.
        """
        c.execute("""SELECT COUNT(p_id)
                FROM results
    ...
    def find_ct_select_phd_cs(self, c):
        """
        Find the count of applicants to selected universities for a PhD in Computer Science.
        """
        c.execute("""SELECT COUNT(p_id)
                FROM results
                WHERE degree = 'PhD' 
    ...
    def find_ct_select_phd_cs_llm(self, c):
        """
        Find the count of applicants to selected universities \
            for a PhD in Computer Science using LLM answers.
        """
        c.execute("""SELECT COUNT(p_id)
                FROM results
                WHERE degree = 'PhD' 
    ...
    def find_prog_most_accept(self, c):
        """
        Find the program with the most acceptances.
        """
        ...
        print(f"Program with most acceptances: \
              {self.prog_most_accept}, {self.prog_accept_ct}")

    def find_prog_most_reject(self, c):
        """
        Find the program with the most rejections.
        """
        c.execute("""SELECT llm_generated_program, COUNT(p_id) as total_count
                FROM results
        ...
        print(f"Program with most rejections: \
              {self.prog_most_reject}, {self.prog_reject_ct}")

    def find_uni_most_accept(self, c):
        """
        Find the university with the most acceptances.
        """
        ...
        if row is None:
            ...
            print(f"University with most acceptances: \
                  {self.uni_most_accept}, {self.uni_accept_ct}")

    def find_uni_most_reject(self, c):
        """
        Find the university with the most rejections.
        """
        c.execute("""SELECT llm_generated_university, COUNT(p_id) as total_count
                FROM results
                WHERE status = 'Rejected'
        ...
        print(f"University with most rejections: \
              {self.uni_most_reject}, {self.uni_reject_ct}")
